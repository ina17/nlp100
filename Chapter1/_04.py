...
def test(target_str="Hi He Lied Because Boron Could Not Oxidize Fluorine. New Nations Might Also Sign Peace Security Clause. Arthur King Can."):
    ...
    for idx, word in enumerate(words_norm):
        if (idx+1) in [1,5,6,7,8,9,15,16,19]:
            elem.update({word[:1]:idx+1})
        else:
            elem.update({word[:2]:idx+1})
    print(elem)
